...
class ReadSubcommand(FileSubcommand):
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        if not args:
            return CommandResult.error("No file path specified. Usage: /file read <path>")
        ...
        try:
            ...
            try:
                ...
                return CommandResult.error(f"Cannot read file as text (binary file?): {args[0]}")
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        try:
            ...
            if not dir_path.exists():
                return CommandResult.error(f"Directory not found: {args[0] if args else 'current directory'}")
            
            if not dir_path.is_dir():
                return CommandResult.error(f"Not a directory: {args[0] if args else 'current directory'}")
            ...
            try:
                for item in dir_path.iterdir():
                    if item.is_dir():
                        items.append((item.name + "/", "DIR", "-", self._format_mtime(item)))
                    else:
                        size = self._format_size(item.stat().st_size)
                        items.append((item.name, "FILE", size, self._format_mtime(item)))
            except PermissionError:
...
class SearchSubcommand(FileSubcommand):
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        ...
        try:
            # Use provided path or current directory
            if len(args) > 1:
                search_path = resolve_path_with_aliases(args[1], context.config.locations)
            else:
            ...
            if not search_path.exists():
                return CommandResult.error(f"Search path not found: {args[1] if len(args) > 1 else 'current directory'}")
                            ...
                            try:
                                with open(item, 'r', encoding='utf-8') as f:
                                    ...
                                    if pattern.lower() in content.lower():
                                        relative_path = item.relative_to(search_path)
                                        results.append((str(relative_path), "content", True))
            ...
            # Display results
            if not results:
                context.console.print(f"No results found for pattern: {pattern}", style="yellow")
...
class CdCommand(BaseCommand):
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        ...
        if not new_dir.exists():
            return CommandResult.error(f"Directory not found: {args[0] if args else 'home'}")
        
        if not new_dir.is_dir():
            return CommandResult.error(f"Not a directory: {args[0] if args else 'home'}")
...
# Filesystem aliases
@command(name="ls", description="List directory contents", usage="/ls [path]", aliases=["dir"])
...
class MkdirCommand(BaseCommand):
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        if not args:
            return CommandResult.error("No directory path specified. Usage: /mkdir <path>")
