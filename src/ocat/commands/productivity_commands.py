...
from ..productivity.models import EntityStatus, EntityType, Task, Event, Reminder, ListItem
...
class ShowTasksCommand(BaseCommand):
    ...
    async def execute(self, args: List[str], context: Any) -> CommandResult:
        ...
        try:
            ...
            if not hasattr(context, "productivity_integration") or context.productivity_integration is None:
            ...
            if category_filter:
                filtered_tasks = [
                    ...
                    if task.category and task.category.lower() == category_filter.lower()
                ]
            ...
            for task in tasks:
                ...
                if hasattr(task, 'priority') and task.priority:
                    if task.priority.lower() == "urgent":
                        ...
                        task_id = f"[bold bright_yellow]{task.pseudo_id}[/bold bright_yellow]"
                ...
                if hasattr(task, 'priority') and task.priority:
                    ...
                    priority_text = priority_map.get(task.priority.lower(), task.priority.upper())
                ...
                if hasattr(task, 'priority') and task.priority:
                    if task.priority.lower() == "urgent":
                        ...
                        task_desc = f"[bold bright_yellow]{task.content}[/bold bright_yellow]"
                ...
                if hasattr(task, 'category') and task.category:
                    ...
                    category_colors = [
                        ...
                        "bright_blue", "magenta", "green", "blue", "cyan"
                    ]
                ...
                table.add_row(task_id, status_emoji, priority_text, category, task_desc, due_date)
        ...
        try:
            ...
            if list_name:
                ...
                for item in filtered_items:
                    ...
                    table.add_row(item.pseudo_id, status_emoji, category, item.content, date_added)
                
                # Display table
                context.console.print(table)
                
                return CommandResult.ok(f"Displayed {len(filtered_items)} items from '{list_name}'")
