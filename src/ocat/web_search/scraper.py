    ...
    async def scrape_url(self, url: str) -> PageContent:
        ...
        try:
            ...
            if not self._is_html_url(url):
                return PageContent(
                    url, "", "", success=False, error="Non-HTML content type"
                )
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status != 200:
                        return PageContent(
                            url, "", "", success=False, error=f"HTTP {response.status}"
                        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                page_contents.append(
                    PageContent(urls[i], "", "", success=False, error=str(result))
                )
    ...
    def _extract_content(self, url: str, html: str) -> PageContent:
        ...
        try:
            ...
            for selector in [
                ...
                if element and hasattr(
                    element, "get_text"
                ):  # Ensure it's a Tag, not NavigableString
            ...
            return PageContent(
                url, "", "", success=False, error=f"Content extraction failed: {e}"
            )
