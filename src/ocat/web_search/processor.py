...
class ProcessedContent:
    ...
    def __init__(self, query: str, results: List[Dict[str, Any]], total_found: int, successful_scrapes: int):
        self.query = query
...
class ContentProcessor:
    ...
    def process_content(self, query: str, page_contents: List[PageContent]) -> ProcessedContent:
        """
        ...
        successful_contents = [pc for pc in page_contents if pc.success and pc.text.strip()]
        self.logger.info(f"Processing {len(successful_contents)} successful page scrapes")
        
        results = []
        for page_content in successful_contents:
            processed_text = self._process_text(page_content.text)
            if processed_text:  # Only include if there's meaningful content after processing
                results.append({
    ...
    def _process_text(self, text: str) -> str:
        ...
        self.logger.debug(f"Truncated content from {len(words)} to ~{len(truncated_text.split())} words")
        
        return truncated_text + "..." if len(words) > self.content_threshold else truncated_text
