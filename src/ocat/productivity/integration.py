...
class ProductivityIntegration:
    ...
    def __init__(self, storage: ProductivityStorage):
        ...
        self.productivity_keywords = {
            # Task-related
            'task', 'todo', 'assignment', 'work', 'project',
            # Event-related 
            'meeting', 'event', 'appointment', 'schedule', 'calendar',
            # Reminder-related
            'remind', 'reminder', 'alert', 'notify',
            # Memory-related
            'remember', 'memory', 'note', 'save', 'store',
            # Actions
            'create', 'add', 'new', 'make', 'set', 'schedule',
            'update', 'edit', 'change', 'modify', 'mark',
            'show', 'list', 'display', 'find', 'search',
            'delete', 'remove', 'cancel', 'complete', 'done',
        }
        ...
        self.compiled_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.productivity_phrases]
    ...
    def should_use_productivity_agent(self, user_input: str) -> bool:
        ...
        keyword_matches = sum(1 for keyword in self.productivity_keywords 
        ...
        if keyword_matches >= 1:
            time_indicators = [
                ...
                'next', 'this', 'at', 'by', 'due', 'deadline', 'o\'clock', 'pm', 'am',
    ...
    async def process_productivity_request(self, user_input: str, chat_session: "ChatSession") -> Optional[str]:
        """
        ...
        try:
            ...
            if chat_session.logger:
                chat_session.logger.info(f"Processed productivity request: {user_input[:50]}...")
...
def create_productivity_integration(chat_session: "ChatSession") -> Optional[ProductivityIntegration]:
    """
    ...
    try:
        ...
        if not chat_session.vector_store:
            chat_session.logger.warning("Productivity disabled: vector store not available")
            return None
