...
class BaseEntity(BaseModel):
    ...
    status: EntityStatus = Field(default=EntityStatus.ACTIVE, description="Current status")
...
class Task(BaseEntity):
    ...
    priority: Optional[Literal["low", "medium", "high", "urgent"]] = Field(None, description="Task priority")
        ...
        if isinstance(v, str):
            try:
                ...
                if v_lower in ['today', 'tonight']:
                    ...
                    tomorrow = datetime.now().replace(hour=23, minute=59, second=59, microsecond=0)
...
class Event(BaseEntity):
    ...
    def validate_datetime_range(self):
        ...
        if self.start_datetime and self.end_datetime and self.end_datetime <= self.start_datetime:
            ...
            self.end_datetime = self.start_datetime.replace(hour=self.start_datetime.hour + 1)
