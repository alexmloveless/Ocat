...
def _format_entity_list_text(entities: List[ProductivityEntity], title: Optional[str] = None) -> str:
    ...
    for i, entity in enumerate(entities):
        ...
        if isinstance(entity, Task) and entity.due_date:
            ...
            details.append(f"Trigger: {_format_datetime_short(entity.trigger_datetime)}")
...
def _format_entity_list_markdown(entities: List[ProductivityEntity], title: Optional[str] = None) -> str:
    ...
    for entity in entities:
        ...
        lines.append(f"| `{entity.pseudo_id}` | {type_emoji} {entity.entity_type.value} | {status_emoji} {entity.status.value} | {entity.content} | {details_text} |")
...
def _format_entity_list_csv(entities: List[ProductivityEntity]) -> str:
    ...
    for entity in entities:
        ...
        if isinstance(entity, Task):
            ...
            details.append(f"Trigger: {entity.trigger_datetime.strftime('%Y-%m-%d %H:%M')}")
        
        details_text = "; ".join(details)
        if ',' in details_text:
            details_text = f'"{details_text}"'
        
        lines.append(f"{entity.pseudo_id},{entity.entity_type.value},{entity.status.value},{content},{created},{updated},{details_text}")
...
def _format_datetime(dt: datetime) -> str:
    ...
    # If it's tomorrow
    elif dt.date() == (now.date().replace(day=now.day + 1) if now.day < 28 else now.date()):
