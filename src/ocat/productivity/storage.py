...
from .models import (
    ProductivityEntity, Task, Event, Reminder, Memory, 
    EntityType, EntityStatus, create_entity
)
...
class ProductivityStorage:
    ...
    def _metadata_to_entity(self, metadata: Dict[str, Any], content: str) -> Optional[ProductivityEntity]:
        ...
        try:
            ...
            entity_data = {
                'content': metadata.get('entity_content', content.split('Assistant: ')[-1] if 'Assistant: ' in content else content),
                'pseudo_id': metadata.get('entity_pseudo_id'),
                'status': metadata.get('entity_status', EntityStatus.ACTIVE),
                'created_at': datetime.fromisoformat(metadata['entity_created_at']) if 'entity_created_at' in metadata else datetime.now(),
                'updated_at': datetime.fromisoformat(metadata['entity_updated_at']) if 'entity_updated_at' in metadata else None,
            ...
            if entity_type == EntityType.TASK:
                entity_data.update({
                    'due_date': datetime.fromisoformat(metadata['task_due_date']) if metadata.get('task_due_date') else None,
                ...
                entity_data.update({
                    'start_datetime': datetime.fromisoformat(metadata['event_start_datetime']),
                    'end_datetime': datetime.fromisoformat(metadata['event_end_datetime']) if metadata.get('event_end_datetime') else None,
                    'all_day': str(metadata.get('event_all_day', 'False')).lower() == 'true',
                    'participants': json.loads(metadata.get('event_participants', '[]')),
                    'location': metadata.get('event_location'),
                })
            elif entity_type == EntityType.REMINDER:
                entity_data.update({
                    'trigger_datetime': datetime.fromisoformat(metadata['reminder_trigger_datetime']),
                    'category': metadata.get('reminder_category'),
                    'recurring': str(metadata.get('reminder_recurring', 'False')).lower() == 'true',
                })
    ...
    def _entity_to_metadata(self, entity: ProductivityEntity) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            'entity_updated_at': entity.updated_at.isoformat() if entity.updated_at else None,
        ...
        if isinstance(entity, Task):
            metadata.update({
                'task_due_date': entity.due_date.isoformat() if entity.due_date else None,
            ...
            metadata.update({
                ...
                'event_end_datetime': entity.end_datetime.isoformat() if entity.end_datetime else None,
                            ...
                            if i < len(metadatas):
                                ...
                                if 'entity_type' in metadata:
                                    ...
                                    if entity_types:
                                        ...
                                        if entity_type_str not in [et.value for et in entity_types]:
                                            continue
