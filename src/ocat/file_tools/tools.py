...
@file_agent.tool
async def file_read_tool(ctx: RunContext[FileStorage], request: FileReadRequest) -> str:
...
@file_agent.tool
async def file_list_tool(ctx: RunContext[FileStorage], request: FileListRequest) -> str:
    ...
    try:
        ...
        result_lines = []
        dir_path = (
            ctx.deps.resolve_path(request.path)
            if request.path
            else ctx.deps.context.current_directory
        )
...
async def file_search_tool(
    ...
    try:
        results = ctx.deps.search_files(
            request.pattern, request.path, request.search_content
        )
        ...
        result_lines.append(
            f"Search results for '{request.pattern}' in {search_location} ({search_type}):"
        )
        result_lines.append("")
...
@file_agent.tool
async def file_tree_tool(ctx: RunContext[FileStorage], request: FileTreeRequest) -> str:
