...
class FileStorage:
    ...
    def list_directory(self, path_str: Optional[str] = None, show_hidden: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        if not dir_path.is_dir():
            raise ValueError(f"Path is not a directory: {path_str or 'current directory'}")
        ...
        try:
            ...
            raise ValueError(f"Permission denied listing directory: {path_str or 'current directory'}")
    
    def search_files(self, pattern: str, path_str: Optional[str] = None, search_content: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        if not search_path.exists():
            raise ValueError(f"Search path not found: {path_str or 'current directory'}")
        ...
        try:
            ...
            if search_content and not any(char in pattern for char in ['*', '?', '[', ']']):
                for item in search_path.rglob("*.txt"):
                    if item.is_file() and item.stat().st_size <= self.context.max_file_size:
                        try:
    ...
    def get_directory_tree(self, path_str: Optional[str] = None, max_depth: int = 3) -> Dict[str, Any]:
        """
