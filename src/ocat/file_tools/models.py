...
class FileReadRequest(BaseModel):
    ...
    path: str = Field(description="File path to read (supports location aliases like 'docs:readme.md')")
...
class FileSearchRequest(BaseModel):
    ...
    pattern: str = Field(description="Search pattern (filename pattern or text content)")
    path: Optional[str] = Field(
        default=None,
...
class FileTreeRequest(BaseModel):
    ...
    max_depth: int = Field(
        default=3,
        description="Maximum depth to show in tree (1-10)"
    )
