...
def resolve_path_with_aliases(path_str: str, locations: Dict[str, str]) -> Path:
    ...
    """
    # Check for standalone alias (alias that itself is a file path)
    if path_str in locations:
        return Path(locations[path_str]).expanduser()
    ...
    if (
        ...
        and not (len(path_str) >= 2 and path_str[1] == ":" and path_str[0].isalpha())
    ):
