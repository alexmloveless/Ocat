...
def restore_chroma_db(backup_path: Path, restore_path: Path, overwrite: bool = False) -> None:
    """
...
def get_backup_info(backup_path: Path) -> dict:
    ...
    info = {
        ...
        "exists": True,
        "type": "compressed" if backup_path.suffix == '.gz' or backup_path.name.endswith('.tar.gz') else "uncompressed",
        "size_bytes": 0
    }
...
Examples:
  ...
  %(prog)s info /path/to/backup.tar.gz
    ...
    backup_parser.add_argument('db_path', type=Path, help='Path to the Chroma database directory')
    backup_parser.add_argument('backup_path', type=Path, help='Path for the backup file/directory')
    backup_parser.add_argument('--no-compress', action='store_true', help='Skip compression')
    
    # Restore command
    restore_parser = subparsers.add_parser('restore', help='Restore a Chroma database from backup')
    restore_parser.add_argument('backup_path', type=Path, help='Path to the backup file/directory')
    restore_parser.add_argument('restore_path', type=Path, help='Path where database will be restored')
    ...
    info_parser.add_argument('backup_path', type=Path, help='Path to the backup file/directory')
