...
def query_collection(db_path: Path, collection_name: str = "conversation", 
                    query_text: str = "", n_results: int = 5) -> None:
    ...
    if metadata_file.exists():
        try:
            ...
            if metadata:
                ...
                import datetime
                earliest_date = datetime.datetime.fromtimestamp(earliest).strftime('%Y-%m-%d %H:%M')
                latest_date = datetime.datetime.fromtimestamp(latest).strftime('%Y-%m-%d %H:%M')
...
def main():
    ...
    query_parser.add_argument("--collection", "-c", default="conversation",
                             help="Collection name (default: conversation)")
    query_parser.add_argument("--text", "-t", default="",
                             help="Query text (if empty, shows first N items)")
